...
public class DrawManager : MonoBehaviour
{
    ...
    {
        input.Enable();
        // Ya no nos suscribimos a eventos; lo leemos en Update
    }
    ...
    void CalculatePixel()//This function checks if the cursor is currently over the canvas and, if it is, it calculates which pixel on the canvas it is on
    {
        ...
        Ray ray = cam.ScreenPointToRay(Mouse.current.position.ReadValue());//Get a ray from the center of the camera to the mouse position
        RaycastHit hit;
